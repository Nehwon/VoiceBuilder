...
class ExplorerPathIn(BaseModel):
    """Un chemin est la liste des ids (segments) de la navigation."""
    path: list[str] = []


class ExplorerNewIn(BaseModel):
    path: list[str] = []
    name: str | None = None


class ExplorerRenameIn(BaseModel):
    path: list[str] = []
    id: str = ""
    newname: str = ""


class ExplorerDeleteIn(BaseModel):
    path: list[str] = []
    ids: list[str] = []
    recycle: bool = False


class ExplorerTransferIn(BaseModel):
    srcpath: list[str] = []
    srcids: list[str] = []
    destpath: list[str] = []
...
# ---------------------------------------------------------------------------
# Explorateur de fichiers (js-fileexplorer) : racines virtuelles
# ---------------------------------------------------------------------------

# Racines virtuelles affichées dans l'explorateur. Les chemins listés par le
# widget sont des segments : le 1er id (après la racine virtuelle "racine")
# désigne une de ces racines, les suivants un sous-dossier relatif.
def _explorer_roots() -> dict:
    return {
        "projets": {"label": "Projets (texte/)", "path": config.TEXTE_DIR},
        "sorties": {"label": "Sorties audio (output/)", "path": config.OUTPUT_DIR},
        "audio":   {"label": "Échantillons voix (audio)", "path": config.VOIX_AUDIO_DIR},
    }


def _explorer_racine() -> dict:
    return {"id": "racine", "label": "VoiceBuilder"}


def _explorer_resoudre(ids: list[str]) -> tuple[str | None, Path]:
    """Résout une liste d'ids (segments du widget) en (racine_id, dossier).

    Les ids commencent éventuellement par la racine virtuelle ``racine`` puis
    par l'id d'une racine réelle. Retourne ``(None, None)`` si le chemin pointe
    sur la racine virtuelle elle-même. Toute sortie de la racine est refusée.
    """
    if not ids:
        return None, None
    segs = ids[1:] if ids[0] == "racine" else ids
    if not segs:
        return None, None
    root_id = segs[0]
    roots = _explorer_roots()
    if root_id not in roots:
        raise HTTPException(400, f"Racine inconnue : {root_id}")
    base = Path(roots[root_id]["path"]).resolve()
    cur = base
    for s in segs[1:]:
        if not s or s in (".", "..") or "/" in s or "\\" in s:
            raise HTTPException(400, f"Segment de chemin invalide : {s!r}")
        cur = (cur / s).resolve()
        if not cur.is_relative_to(base):
            raise HTTPException(400, f"Chemin hors de la racine : {s!r}")
    return root_id, cur


def _explorer_entry(p: Path) -> dict:
    """Convertit un fichier/dossier du disque en entrée compatible js-fileexplorer."""
    try:
        st = p.stat()
    except OSError:
        st = None
    taille = st.st_size if st else 0
    modifie = int(st.st_mtime) if st else 0
    base = {"id": p.name, "name": p.name, "hash": p.name + "/",
            "type": "folder", "size": taille}
    if p.is_dir():
        base["attrs"] = {"modifie": modifie}
        return base
    base["type"] = "file"
    base["hash"] = p.name
    base["attrs"] = {"modifie": modifie}
...
def _explorer_nom_unique(dir_: Path, name: str, is_dir: bool) -> str:
    """Renvoie un nom libre dans ``dir_`` (suffixe ``_2``, ``_3``…)."""
    if not name or name.startswith("."):
        name = "Nouveau dossier" if is_dir else "nouveau-fichier.txt"
...
def api_explorer_list(path: str = "[]"):
    """Liste le contenu d'un dossier (ou les racines virtuelles)."""
    try:
        ids = json.loads(path)
    ...
    if not isinstance(ids, list):
        raise HTTPException(400, "Chemin invalide.")
    root_id, cur = _explorer_resoudre(ids)
    if root_id is None:
        entries = []
        for rid, meta in _explorer_roots().items():
            entries.append({"id": rid, "name": meta["label"], "hash": rid + "/",
                            "type": "folder", "size": 0,
                            "attrs": {"racine": True}})
        return {"path": ids, "entries": entries}
    if not cur.is_dir():
        return {"path": ids, "entries": []}
    entries = []
    try:
        noms = sorted(os.listdir(cur))
    except OSError as exc:
    ...
    for name in noms:
        ...
        p = cur / name
        try:
            entries.append(_explorer_entry(p))
        except OSError:
            continue
    return {"path": ids, "entries": entries}


@app.get("/api/explorer/racine")
def api_explorer_racine():
    return _explorer_racine()


@app.post("/api/explorer/newfolder")
def api_explorer_newfolder(payload: ExplorerNewIn):
    root_id, cur = _explorer_resoudre(payload.path)
    if root_id is None:
        raise HTTPException(400, "Impossible de créer à la racine virtuelle.")
    cur.mkdir(parents=True, exist_ok=True)
    name = _explorer_nom_unique(cur, payload.name or "", is_dir=True)
    dest = cur / name
    dest.mkdir(parents=True)
    return {"entry": _explorer_entry(dest)}


@app.post("/api/explorer/newfile")
def api_explorer_newfile(payload: ExplorerNewIn):
    root_id, cur = _explorer_resoudre(payload.path)
    if root_id is None:
        raise HTTPException(400, "Impossible de créer à la racine virtuelle.")
    cur.mkdir(parents=True, exist_ok=True)
    name = _explorer_nom_unique(cur, payload.name or "", is_dir=False)
    dest = cur / name
    dest.touch()
    return {"entry": _explorer_entry(dest)}
...
def api_explorer_rename(payload: ExplorerRenameIn):
    root_id, cur = _explorer_resoudre(payload.path)
    if root_id is None:
        raise HTTPException(400, "Chemin invalide.")
    src = (cur / payload.id).resolve()
    if not src.is_relative_to(cur.resolve()) or not src.exists():
        raise HTTPException(404, f"Élément introuvable : {payload.id}")
    nouveau = payload.newname.strip().replace("\\", "/").split("/")[-1]
    if not nouveau or nouveau in (".", "..") or nouveau.startswith("."):
        raise HTTPException(400, "Nouveau nom invalide.")
    dest = cur / nouveau
    if dest.resolve() == src.resolve():
        return {"entry": _explorer_entry(src)}      # aucun changement
    if dest.exists():
        raise HTTPException(409, f"Un élément « {nouveau} » existe déjà.")
    try:
        src.rename(dest)
    except OSError as exc:
        raise HTTPException(500, f"Renommage échoué : {exc}")
...
def _supprimer_element(p: Path) -> None:
...
def api_explorer_delete(payload: ExplorerDeleteIn):
    root_id, cur = _explorer_resoudre(payload.path)
    if root_id is None:
        raise HTTPException(400, "Chemin invalide.")
    for ident in payload.ids:
        p = (cur / ident).resolve()
        if not p.is_relative_to(cur.resolve()) or not p.exists():
            continue
        try:
            _supprimer_element(p)
        except OSError as exc:
            raise HTTPException(500, f"Suppression de « {ident} » échouée : {exc}")
    return {"success": True}


@app.post("/api/explorer/copy")
def api_explorer_copy(payload: ExplorerTransferIn):
    return _explorer_transfert(payload, move=False)


@app.post("/api/explorer/move")
def api_explorer_move(payload: ExplorerTransferIn):
    return _explorer_transfert(payload, move=True)


def _explorer_transfert(payload: ExplorerTransferIn, move: bool) -> dict:
    src_root, src_dir = _explorer_resoudre(payload.srcpath)
    dest_root, dest_dir = _explorer_resoudre(payload.destpath)
    if src_root is None or dest_root is None:
        raise HTTPException(400, "Chemin invalide.")
    ...
    if move and src_dir == dest_dir:
        raise HTTPException(400, "La destination est identique à la source.")
    resultats = []
    for ident in payload.srcids:
        src = (src_dir / ident).resolve()
        if not src.is_relative_to(src_dir) or not src.exists():
            continue
        # refus : copier/déplacer un dossier dans lui-même
        if dest_dir.is_relative_to(src):
            raise HTTPException(400, f"Impossible de {('déplacer' if move else 'copier')} « {ident} » dans lui-même.")
        name = _explorer_nom_unique(dest_dir, src.name, src.is_dir())
        dest = dest_dir / name
        try:
            if move:
            ...
            resultats.append(_explorer_entry(dest))
        except OSError as exc:
            raise HTTPException(500, f"Échec pour « {ident} » : {exc}")
    return {"success": True, "entries": resultats}


_EXT_AUDIO = (".wav", ".mp3", ".flac", ".ogg", ".m4a", ".aac", ".opus")
_EXT_IMAGE = (".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".bmp")
_EXT_TEXTE = (".md", ".txt", ".json", ".csv", ".yml", ".yaml", ".map",
              ".xml", ".html", ".ini", ".cfg", ".log")
_LECTURE_MAX = 512 * 1024  # octets lus pour un aperçu texte


def _explorer_kind(name: str) -> str:
...
def api_explorer_read(path: str = "[]", id: str = ""):
    """Aperçu d'un fichier : contenu texte (tronqué) sinon description."""
    try:
        ids = json.loads(path)
    except (ValueError, TypeError):
        raise HTTPException(400, "Chemin invalide.")
    ...
    if root_id is None:
        raise HTTPException(400, "Chemin invalide.")
    p = (cur / id).resolve()
    if not p.is_relative_to(cur.resolve()) or not p.is_file():
        raise HTTPException(404, f"Fichier introuvable : {id}")
    kind = _explorer_kind(p.name)
    st = p.stat()
    base = {"name": p.name, "kind": kind, "taille": st.st_size,
            "modifie": int(st.st_mtime),
            "racine": root_id, "dossier": "/".join(ids[1:])}
    if kind != "text":
        return base
    try:
        data = p.read_bytes()[:_LECTURE_MAX + 1]
        try:
            texte = data[:_LECTURE_MAX].decode("utf-8")
        except UnicodeDecodeError:
            texte = data[:_LECTURE_MAX].decode("latin-1", errors="replace")
        base["tronque"] = len(data) > _LECTURE_MAX
        base["contenu"] = texte
    except OSError as exc:
        raise HTTPException(500, f"Lecture impossible : {exc}")
    return base


@app.get("/api/explorer/raw")
def api_explorer_raw(path: str = "[]", id: str = ""):
    """Télécharge un fichier brut (lecture audio/aperçu)."""
    try:
        ids = json.loads(path)
    except (ValueError, TypeError):
        raise HTTPException(400, "Chemin invalide.")
...
async def api_explorer_download(request: Request):
    """Téléchargement (form) : un fichier seul, ou un zip pour plusieurs / dossiers."""
    form = await request.form()
    path = form.get("path", "[]")
    ids = form.getlist("id") or []
    try:
        path_ids = json.loads(path)
    except (ValueError, TypeError):
        raise HTTPException(400, "Chemin invalide.")
    root_id, cur = _explorer_resoudre(path_ids)
    if root_id is None:
        raise HTTPException(400, "Chemin invalide.")
    elements = []
    for ident in ids:
        p = (cur / ident).resolve()
        if p.is_relative_to(cur.resolve()) and p.exists():
            elements.append(p)
    if not elements:
        raise HTTPException(404, "Aucun élément à télécharger.")
    if len(elements) == 1 and elements[0].is_file():
        return FileResponse(elements[0], filename=elements[0].name)

    # zip mémoire des éléments sélectionnés (dossiers inclus récursivement)
    fd, tmp = tempfile.mkstemp(suffix=".zip", prefix="vb_download_")
    os.close(fd)
    noms_utilises: set = set()

    def _arc_nom(p: Path) -> str:
        nom = p.name
    ...
    def _ajouter_zip(zf, p: Path):
        if p.is_dir():
            arc = _arc_nom(p) + "/"
            for enfant in sorted(p.rglob("*"), key=lambda e: str(e)):
                rel = enfant.relative_to(p)
                nom_arc = arc + "/".join(rel.parts)
                if enfant.is_dir():
                    zf.writestr(nom_arc + "/", "")
                else:
                    zf.write(enfant, nom_arc)
        else:
            zf.write(p, _arc_nom(p))
    ...
    try:
        ...
        raise HTTPException(500, f"Compression échouée : {exc}")

    def _nettoyer():
        Path(tmp).unlink(missing_ok=True)

    nom_zip = (elements[0].name if len(elements) == 1 else "telechargement") + ".zip"
    return FileResponse(tmp, media_type="application/zip", filename=nom_zip,
                        background=BackgroundTask(_nettoyer))


# Upload (non découpé par défaut ; découpage optionnel compatible Content-Range)
_UPLOADS_PARTS: dict = {}  # uploadid -> {"fh", "dest", "name", "total", "written"}


def _parcourir_relative(relpath: str, cur: Path) -> Path:
    """Résout un chemin relatif d'upload dans ``cur`` (anti-traversal)."""
    segs = [s for s in relpath.replace("\\", "/").split("/") if s not in ("", ".")]
    dest = cur.resolve()
    for s in segs[:-1]:
        if s in ("..",) or "/" in s or "\\" in s:
            raise HTTPException(400, f"Chemin relatif invalide : {s!r}")
        dest = (dest / s).resolve()
        if not dest.is_relative_to(cur.resolve()):
            raise HTTPException(400, "Chemin hors du dossier de destination.")
    if not segs:
        raise HTTPException(400, "Nom de fichier manquant.")
    nom = segs[-1]
    if nom in ("", ".", "..") or nom.startswith("."):
        raise HTTPException(400, f"Nom de fichier invalide : {nom!r}")
    dest.mkdir(parents=True, exist_ok=True)
    return dest / nom


@app.post("/api/explorer/upload")
async def api_explorer_upload(request: Request,
                              file: UploadFile = File(...),
                              path: str = Form(default="[]"),
                              relpath: str | None = Form(default=None),
                              uploadid: str | None = Form(default=None)):
    """Reçoit un (ou des) morceaux d'un fichier et l'écrit dans la racine visée.

    - Sans en-tête ``Content-Range`` : fichier complet en un POST.
    - Avec ``Content-Range: bytes A-B/TOTAL`` : écriture par morceaux (le
      client rejoue le même intervalle en cas de retry → écriture idempotente).
    Le paramètre ``relpath`` transporte le nom (relatif) du fichier.
    """
    try:
        ids = json.loads(path)
    except (ValueError, TypeError):
    ...
    relname = relpath or file.filename or "fichier.bin"
    if relname.startswith("/"):
        relname = relname[1:]
    dest = _parcourir_relative(relname, cur)

    cr = request.headers.get("content-range", "")
    m = re.match(r"bytes\s+(\d+)-(\d+)/(\d+)$", cr) if cr else None

    if not m:
        # upload complet en un seul morceau
        try:
            ...
            raise HTTPException(500, f"Écriture échouée : {exc}")
        return {"success": True, "entry": _explorer_entry(dest)}

    start, _end, total = int(m.group(1)), int(m.group(2)), int(m.group(3))
    key = uploadid or f"{root_id}-{dest.name}"
    data = await file.read()
    if total == 0:
        # fichier vide
        dest.touch()
        return {"success": True, "entry": _explorer_entry(dest)}
    if key not in _UPLOADS_PARTS:
        dest.parent.mkdir(parents=True, exist_ok=True)
        part = Path(tempfile.gettempdir()) / f"vb_part_{key}.part"
        fh = open(part, "w+b")
        _UPLOADS_PARTS[key] = {"fh": fh, "dest": dest, "part": part,
                               "total": total, "written": 0}
    rec = _UPLOADS_PARTS[key]
    fh = rec["fh"]
    fh.seek(start)
    fh.write(data)
    fh.flush()
    rec["written"] = max(rec["written"], start + len(data))
    if rec["written"] < rec["total"]:
        return {"success": True}
    # dernier morceau → déplace vers la destination finale
    fh.close()
    try:
        shutil.move(str(rec["part"]), str(rec["dest"]))
    except OSError as exc:
        raise HTTPException(500, f"Finalisation de l'upload échouée : {exc}")
    finally:
        rec.pop("fh", None)
        _UPLOADS_PARTS.pop(key, None)
    return {"success": True, "entry": _explorer_entry(rec["dest"])}
